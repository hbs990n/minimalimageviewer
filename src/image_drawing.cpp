                ...
                if (useHighRes) {
                    ...
                    if (SUCCEEDED(m_ctx.renderTarget->QueryInterface(IID_PPV_ARGS(&dc5)))) {
                        ...
                        if (!m_ctx.highResImageSource) {
                            ...
                            if (SUCCEEDED(m_ctx.wicFactory->CreateStream(&freshStream)) &&
                                SUCCEEDED(freshStream->InitializeFromMemory(
                                    m_ctx.rawFileData.data(),
                                    static_cast<DWORD>(m_ctx.rawFileData.size()))))
                            {
                                ComPtr<IWICBitmapDecoder> decoder;
                                if (SUCCEEDED(m_ctx.wicFactory->CreateDecoderFromStream(freshStream.Get(), NULL, WICDecodeMetadataCacheOnLoad, &decoder))) {
